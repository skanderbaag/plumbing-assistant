...
async function getJobsInRange(minISO, maxISO) {
    let jobs = [];
    let page = 1;
    while (true) {
        const data = await hcFetch(`/jobs?scheduled_start_min=${minISO}&scheduled_start_max=${maxISO}&page_size=100&page=${page}`);
...
async function getJobsInWideWindow() {
    ...
    return getJobsInRange(min, max);
}

// Fetches every job the company has ever had in Housecall Pro. Used for reports, where a
// recall needs to be traced back to whatever original job it stems from, which could be
// months (or longer) before the recall itself. First page tells us total_pages, then the
// rest are fetched in parallel - in practice this is ~5-6k jobs and takes a few seconds.
async function getAllJobsHistory() {
    const min = '2015-01-01T00:00:00Z';
    const max = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    const first = await hcFetch(`/jobs?scheduled_start_min=${min}&scheduled_start_max=${max}&page_size=100&page=1`);
    const totalPages = first.total_pages || 1;
    const rest = await Promise.all(
        Array.from({ length: totalPages - 1 }, (_, i) =>
            hcFetch(`/jobs?scheduled_start_min=${min}&scheduled_start_max=${max}&page_size=100&page=${i + 2}`)
        )
    );
    return [first, ...rest].flatMap(d => d.jobs || []);
}
